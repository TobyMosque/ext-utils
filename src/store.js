...
const mapStoreCollections = function (collections) {
  ...
  for (let collection of collections) {
    ...
    getters[`${plural.camel}Index`] = function (state, item) {
      return state[item.plural].reduce((map, item, indice) => {
        map.set(item[collection.id], indice)
        return map
      }, new Map())
    }
    getters[`${single.camel}ById`] = function (state, index) {
      return (id) => {
        let index = getters[`${plural.camel}Index`].get(id)
        return index ? state[item.plural][index] : null
      }
    }
  }
